...
export type PetsdbQueryType<ItemType extends Record<string, unknown>> = Partial<{
    [Key in keyof ItemType]: ItemType[Key] extends Array<string> | string ? ItemType[Key] | RegExp : ItemType[Key] extends Record<string, unknown> ? PetsdbQueryType<ItemType[Key]> : ItemType[Key];
}>;
...
export type PetsdbSortType<ItemType extends Record<string, unknown>> = Partial<{
    [Key in keyof ItemType]: ItemType[Key] extends PetsdbSortValueType ? PetsdbSortDirectionType : ItemType[Key] extends Record<string, unknown> ? PetsdbSortType<ItemType[Key]> : never;
}>;
