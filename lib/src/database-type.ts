export type TsdbItemType<DataType> = Readonly<DataType & {_id: string}>;
...
export type TsdbSortType<ItemType extends Record<string, unknown>> = Partial<{
    [Key in keyof ItemType]: ItemType[Key] extends TsdbSortValueType
        ? TsdbSortDirectionType
        : ItemType[Key] extends Record<string, unknown>
        ? TsdbSortType<ItemType[Key]>
        : never;
}>;
